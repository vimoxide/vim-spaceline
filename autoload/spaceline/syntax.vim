...
function! spaceline#syntax#icon_syntax() abort
    let icon = substitute(WebDevIconsGetFileTypeSymbol(), "\u00A0", '', '')
    let bg_color = substitute(synIDattr(hlID("FileName"), "bg"),'#','','')

    if strlen(bg_color) < 6
      let bg_color = substitute(synIDattr(hlID("FileNameRight"), "bg"),'#','','')
    endif
    
    for color in keys(g:coldevicons_iconmap)
        let index = index(g:coldevicons_iconmap[color], icon)
        if index != -1
          execute 'highlight! FileIcon'.color.' guifg='.g:coldevicons_colormap[color].' guibg=#'.bg_color.' ctermfg='.s:rgb(g:coldevicons_colormap[color]) . ' '
          break
        endif
    endfor
